...
p2=animatedline(Ax,Ay,'Color','[0.8 1.0 0.4]','LineWidth',2,'MaximumNumPoints',5);   %다각형을 그리는 함수
...
for i=1:5:n-1
    ...
    draw_animation((x(i)+l*sin(setha(i))),l*cos(setha(i)));
    F=getframe(gcf);
    writeVideo(video,F);

    draw_animation_cartpole((x(i)+l*sin(setha(i))),l*cos(setha(i)),x(i));
    F=getframe(gcf);
    writeVideo(video,F);
end
